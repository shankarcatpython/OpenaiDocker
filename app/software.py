...
def create_database():
    try:
        if not os.path.exists('software_database.db'):
...
def insert_embedding(chunk,embedding_vector):
    try:
        ...
        cursor.execute("INSERT INTO software (content,vector) VALUES (?,?)", (chunk,embedding_json))
        conn.commit()
        conn.close()
...
def retrieve_software():
    try:
        ...
        for row in rows:
            ...
            software_list.append(retrieved_embedding_vector)

        conn.close()
        return content_list,software_list
    except Exception as e:
        print("Error retrieving software: " + str(e))
        return [],[]
...
def generate_prompt_for_llm(user_input):
    # Retrieve all software from the database and create a snapshot
    content , arrayvector = retrieve_software()
...
if __name__ == "__main__":
    ...
    create_database()
    final_data = content_curation(r'SofwareEngineering.pdf')
